...
<div class="ride_driver_candidate_div col-span-3 flex flex-col border border-gray-400 divide-y divide-gray-400 mb-2">
    <div class="flex flex-col bg-white py-2 px-3">
        ...
        <select class="w-full text-sm border border-gray-400 bg-gray-200" disabled>
            ...
            @foreach($drivers as $driver)
                <option value="{{ $driver->user_no }}" @selected((string)old("user_no.{$rideDriverCandidate->user_no}", $rideDriverCandidate->user_no) === (string)$driver->user_no)>{{ $driver->full_name }}</option>
            @endforeach
        </select>
        <input type="hidden" name="user_no[{{ $rideDriverCandidate->ride_driver_candidate_id }}]" value="{{ $rideDriverCandidate->user_no }}">
    </div>
    <div class="flex flex-col bg-white py-2 px-3">
    ...
    <div class="flex flex-col bg-white py-2 px-3">
        ...
        <input type="text" name="driver_memo[{{ $rideDriverCandidate->ride_driver_candidate_id }}]" class="pl-3 w-full text-sm py-2.5 border border-gray-400" value="{{ old('driver_memo.' . $rideDriverCandidate->ride_driver_candidate_id, $rideDriverCandidate->driver_memo) }}" autocomplete="off">
    </div>
