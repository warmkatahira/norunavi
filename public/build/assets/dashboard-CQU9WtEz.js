import{s as w}from"./loading-CLXJ3Lsj.js";$(document).on("click",function(e){e.target.classList.contains("ride_schedule_select_modal_close")&&$("#ride_schedule_select_modal").addClass("hidden");const s=$(e.target).closest(".ride_schedule_select_modal_open");if(s.length){const a=s.data("route-type-id"),l=s.data("ride-id"),r=s.data("join-ride-detail-id");v(a,l,r),$("#ride_id").val(l),$("#ride_schedule_select_modal").removeClass("hidden")}});function v(e,s,a){$.ajax({headers:{"X-CSRF-TOKEN":$('meta[name="csrf-token"]').attr("content")},url:"/ajax/get_ride_schedule_select_info",type:"GET",data:{ride_id:s},dataType:"json",success:function(r){try{e===1?$("#ride_schedule_select_modal_title").html("乗車する場所を選択して下さい"):e===2&&$("#ride_schedule_select_modal_title").html("降車する場所を選択して下さい"),$("#ride_schedule_select_div").empty();let c=$('<div class="hidden md:block overflow-x-auto w-full"></div>'),o=$('<table class="text-sm w-full border-collapse"></table>'),h=$(`
                ...
                `),i=$('<tbody class="bg-white"></tbody>'),n=$('<div class="md:hidden space-y-2 w-full"></div>');r.ride_details.forEach(function(t){let u=y(t),d=!1;e===1?d=!!t.departure_time:e===2&&(d=!!t.arrival_time);let p="",_=t.ride_detail_id===a?"checked":"";d?p=`
                            <input type="radio"
                                name="ride_detail_id"
                        ...
                        `;let b=d?"select-row hover:bg-theme-sub cursor-pointer":"bg-gray-100 text-gray-400 cursor-not-allowed",f=$(`
                        <tr data-id="${t.ride_detail_id}"
                            class="${b} border hover:bg-theme-sub cursor-pointer">
                    ...
                    `);i.append(f);let x=d?"w-full select-row ride-card border rounded-xl p-3 shadow-sm hover:bg-theme-sub cursor-pointer":"w-full ride-card border rounded-xl p-3 shadow-sm bg-gray-100 text-gray-400 cursor-not-allowed",g=$(`
