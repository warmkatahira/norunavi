import{s as w}from"./loading-CLXJ3Lsj.js";$(document).on("click",function(e){e.target.classList.contains("ride_schedule_select_modal_close")&&$("#ride_schedule_select_modal").addClass("hidden");const s=$(e.target).closest(".ride_schedule_select_modal_open");if(s.length){const l=s.data("route-type-id"),a=s.data("ride-id"),r=s.data("join-ride-detail-id");y(l,a,r),$("#ride_id").val(a)}});function y(e,s,l){$.ajax({headers:{"X-CSRF-TOKEN":$('meta[name="csrf-token"]').attr("content")},url:"/ajax/get_ride_schedule_select_info",type:"GET",data:{ride_id:s},dataType:"json",success:function(r){try{e===1?$("#ride_schedule_select_modal_title").html("乗車する場所を選択して下さい"):e===2&&$("#ride_schedule_select_modal_title").html("降車する場所を選択して下さい"),$("#ride_schedule_select_div").empty();let c=$('<div class="hidden md:block overflow-x-auto w-full"></div>'),i=$('<table class="text-sm w-full border-collapse"></table>'),h=$(`
                    <thead>
                ...
                `),o=$('<tbody class="bg-white"></tbody>'),n=$('<div class="md:hidden space-y-2 w-full"></div>');r.ride_details.forEach(function(t){let p=v(t),d=!1;e===1?d=!!t.departure_time:e===2&&(d=!!t.arrival_time);let u="",_=t.ride_detail_id===l?"checked":"";d?u=`
                        ...
                        <tr data-id="${t.ride_detail_id}"
                            ...
                            <td class="py-1 px-2 text-center">${p}</td>
                        </tr>
                    ...
                    `);n.append(g)}),i.append(h),i.append(o),c.append(i),$("#ride_schedule_select_div").append(c).append(n),l?($("#selected_ride_detail_id").val(l),$(`.select-row[data-id="${l}"]`).trigger("click")):$(".select-row").removeClass("bg-theme-sub").addClass("bg-white"),$("#ride_schedule_select_modal").removeClass("hidden")}catch{}},error:function(){alert("失敗")}})}function v(e){let s=e.departure_time?m(e.departure_time):null,l=e.arrival_time?m(e.arrival_time):null,a="";return l&&s?a=`<span class="text-orange-700 font-medium">${l} 着</span>
                    <span class="mx-1">→</span>
                    <span class="text-blue-700 font-medium">${s} 発</span>`:l?a=`<span class="text-orange-700 font-medium">${l} 着</span>`:s?a=`<span class="text-blue-700 font-medium">${s} 発</span>`:a='<span class="text-gray-400">—</span>',a}function m(e){return e?e.split(":").slice(0,2).join(":"):null}$(document).on("click",".select-row",function(){if($(this).hasClass("cursor-not-allowed"))return;let e=$(this).data("id");$("#selected_ride_detail_id").val(e),$(".select-row").removeClass("bg-theme-sub").addClass("bg-white"),$(`.select-row[data-id="${e}"]`).removeClass("bg-white").addClass("bg-theme-sub")});$("#ride_schedule_select_enter").on("click",function(){window.confirm("選択を確定しますか？")===!0&&(w(),$("#ride_schedule_select_form").submit())});
