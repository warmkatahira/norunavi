$(document).on("click",function(e){e.target.classList.contains("ride_schedule_check_modal_close")&&$("#ride_schedule_check_modal").addClass("hidden");const t=$(e.target).closest(".ride_schedule_check_modal_open");if(t.length){const l=t.data("route-type-id"),s=t.data("ride-id");t.data("join-ride-detail-id"),k(l,s),$("#ride_id").val(s)}});function k(e,t,l){$.ajax({headers:{"X-CSRF-TOKEN":$('meta[name="csrf-token"]').attr("content")},url:"/ajax/get_ride_schedule_select_info",type:"GET",data:{ride_id:t},dataType:"json",success:function(c){try{$("#ride_schedule_check_div").empty();let n=$('<div class="hidden md:block overflow-x-auto w-full"></div>'),p=`<div class="mb-3 text-xl"><i class="las la-calendar la-lg mr-1"></i>${c.schedule_date}</div>`,o=`<div class="mb-3 text-base"><i class="las la-comment la-lg mr-1"></i>${c.ride.ride_memo}</div>`,x=$(p),f=$(p),y=$(o),b=$(o),r=$('<table class="text-sm w-full border-collapse"></table>'),v=$(`
                    <thead>
